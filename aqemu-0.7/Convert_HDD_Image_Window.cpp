...
void Convert_HDD_Thread::run()
{
	...
	proc->start( settings.value("QEMU_IMG_Path", "qemu-img").toString(), Arguments );
	...
	{
		AQDebug( "void Convert_HDD_Thread::run()",
				 "Conversion Complete!" );
...
void Convert_HDD_Image_Window::on_Button_Browse_Base_clicked()
{
	QString fileName = QFileDialog::getOpenFileName( this,
			tr("Select Base HDD Image File"), QDir::homePath(), tr("All Files (*)") );
...
void Convert_HDD_Image_Window::on_Button_Browse_Output_clicked()
{
	QString fileName = QFileDialog::getSaveFileName( this,
			tr("Save Out HDD Image File"), QDir::homePath(), tr("All Files (*)") );
...
void Convert_HDD_Image_Window::on_Button_Convert_clicked()
{
	if( ui.Edit_Output_File_Name->text().isEmpty() )
	{
		AQGraphic_Warning( tr("File Name is Empty!"),
						   tr("Please Enter Correct File Name for You HDD Image!") );
		return;
	}
